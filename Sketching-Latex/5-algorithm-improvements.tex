...
The root cause was identified in the Joint Diagonalization step. The standard JESPRIT formulation assumes that the Rotational Invariance Matrices $\bm{\Psi}_l$ for different directions $l=1, \dots, M$ can be jointly diagonalized by a single unitary matrix. However, this assumption holds only if the steering vectors are orthogonal or if the matrices share a common eigenbasis. In the general case of mixed Poisson estimation with arbitrary parameters, the matrices $\bm{\Psi}_l$ are not normal and do not necessarily share a common unitary basis.

Specifically, the ESPRIT step for each direction $l$ produces a matrix $\bm{\Psi}_l$ such that its eigenvalues correspond to the desired frequency components $\exp(j \delta \bm{u}_l^\top \bm{\omega}_k)$. However, the eigenvectors of $\bm{\Psi}_l$ depend on the specific direction $\bm{u}_l$ and the mixing matrix, which varies with $l$. Consequently, forcing a single unitary matrix to diagonalize all $\bm{\Psi}_l$ simultaneously leads to a compromise solution that mixes the components, resulting in the observed bias.
