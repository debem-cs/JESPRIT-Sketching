...
The signal at any $d$-dimensional coordinate $\boldsymbol{n}$ is sampled with a grid scale $\Delta$:
$$ \boldsymbol{y}[\boldsymbol{n}] = \sum_{k=1}^{r} a_k e^{j\langle \Delta \boldsymbol{\omega}_k, \boldsymbol{n} \rangle} + \boldsymbol{w}[\boldsymbol{n}] $$
where:
\begin{itemize}
    \item $a_k \in \mathbb{C}$ is the complex amplitude of component $k$.
    \item $\boldsymbol{\omega}_k$ is the unknown $d$-dimensional frequency vector we want to find.
    \item $\Delta$ is a known grid scale factor.
...
\begin{align*}
    z_{l,s}[n] &= \sum_{k=1}^{r} a_k e^{j\langle \Delta \boldsymbol{\omega}_k, \boldsymbol{p}_{l,s} + n \boldsymbol{u}_l \rangle} + w_{l,s}[n] \\
    z_{l,s}[n] &= \sum_{k=1}^{r} \left( a_k e^{j\langle \Delta \boldsymbol{\omega}_k, \boldsymbol{p}_{l,s} \rangle} \right) \left( e^{j\varphi_{k,l}} \right)^n + w_{l,s}[n]
\end{align*}
This is a standard 1D multi-snapshot model. The term $\varphi_{k,l} = \Delta \langle \boldsymbol{\omega}_k, \boldsymbol{u}_l \rangle$ is the 1D frequency for this direction; it is the scaled scalar projection of the $d$-D frequency vector $\boldsymbol{\omega}_k$ onto the 1D direction vector $\boldsymbol{u}_l$.
...
\end{pmatrix} \quad \text{where } \varphi_{k,l} = \Delta \langle \boldsymbol{\omega}_k, \boldsymbol{u}_l \rangle  \\[5ex]
 \mathbf{C}_l &= \begin{pmatrix}
a_{1}e^{j\langle \Delta \boldsymbol{\omega}_{1},\boldsymbol{p}_{1,l}\rangle} & a_{1}e^{j\langle \Delta \boldsymbol{\omega}_{1},\boldsymbol{p}_{2,l}\rangle} & \cdots & a_{1}e^{j\langle \Delta \boldsymbol{\omega}_{1},\boldsymbol{p}_{S_l,l}\rangle} \\
a_{2}e^{j\langle \Delta \boldsymbol{\omega}_{2},\boldsymbol{p}_{1,l}\rangle} & a_{2}e^{j\langle \Delta \boldsymbol{\omega}_{2},\boldsymbol{p}_{2,l}\rangle} & \cdots & a_{2}e^{j\langle \Delta \boldsymbol{\omega}_{2},\boldsymbol{p}_{S_l,l}\rangle} \\
\vdots & 
\vdots & \ddots & \vdots \\
a_{r}e^{j\langle \Delta \boldsymbol{\omega}_{r},\boldsymbol{p}_{1,l}\rangle} & a_{r}e^{j\langle \Delta \boldsymbol{\omega}_{r},\boldsymbol{p}_{2,l}\rangle} & \cdots & a_{r}e^{j\langle \Delta \boldsymbol{\omega}_{r},\boldsymbol{p}_{S_l,l}\rangle}
\end{pmatrix} 
\end{align*}
...
\begin{enumerate}
    ...
    $$ \hat{\boldsymbol{\omega}}_k = \frac{1}{\Delta} (\mathbf{U}^T \mathbf{U})^{-1} \mathbf{U}^T \boldsymbol{\tilde{\varphi}}_k $$
\end{enumerate}
...
z[\boldsymbol{n}] - \sum_{k=1}^r a_k e^{j\langle \Delta \hat{\boldsymbol{\omega}}_k, \boldsymbol{n} \rangle} \right|^2 $$
