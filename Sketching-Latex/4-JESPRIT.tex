...
Below is the pseudocode for the JESPRIT algorithm as implemented in \texttt{jesprit.py}, incorporating the Global SVD adjustment.

\begin{algorithm}
\caption{JESPRIT Estimation Algorithm}
\begin{algorithmic}[1]
\Require Measurement Data $X$, Rank $r$, Number of directions $M$, Samples $S$, Points per line $N$, Grid scale $\Delta$
\Ensure Estimated poisson rates $\hat{\bm{A}}$, Estimated latent factors probabilities $\hat{\bm{\pi}}$

\State \textbf{Step 1: PGF Sampling}
\State Compute PGF samples $Z_l \in \mathbb{C}^{N \times S}$ for $l=1,\dots,M$
\State Store direction vectors $U_{\text{dir}}$ and base points $P_{\text{base}}$

\State \textbf{Step 2: Global Subspace Estimation}
\State $X_{\text{glob}} \gets \text{stack}(Z_1, Z_2, \dots, Z_M)$ \Comment{Vertical concatenation}
\State $U_{\text{glob}}, \Sigma, V^H \gets \text{SVD}(X_{\text{glob}})$
\State $U_s \gets U_{\text{glob}}(:, 1:r)$ \Comment{Extract dominant $r$ left singular vectors}

\State \textbf{Step 3: Compute Rotational Invariance Matrices (RIMs)}
\For{$l = 1$ to $M$}
    \State $U_{\text{line}} \gets U_s[(l-1)N+1 : lN, :]$ \Comment{Extract $N \times r$ block}
    \State $U_1 \gets U_{\text{line}}(1:N-1, :)$
    \State $U_2 \gets U_{\text{line}}(2:N, :)$
    \State $\Psi_l \gets U_1^\dagger U_2$ \Comment{Solve $U_1 \Psi_l \approx U_2$}
\EndFor

\State \textbf{Step 4: Joint Diagonalization}
\State $\mathcal{D} \gets \text{JointDiag}([ \Psi_1, \dots, \Psi_M ])$
\State Extract eigenvalues $\mu_{l,k}$ from diagonal blocks of $\mathcal{D}$

\State \textbf{Step 5: Parameter Recovery}
\State $\Phi \gets \text{angle}(\mu)$ \Comment{Recover phases}
\State $\hat{\bm{\pi}} \gets \frac{1}{\Delta} (U_{\text{dir}}^\dagger \Phi)$ \Comment{Solve $\Delta U_{\text{dir}} \hat{\bm{\pi}} \approx \Phi$}

\State \textbf{Step 6: Rate Estimation}
\State Let $\{\boldsymbol{x}_i\}$ be the set of all sample locations $\boldsymbol{p}_{l,s} + n\boldsymbol{u}_l$
\State Form $A_{\text{glob}}$ where $(A_{\text{glob}})_{i,k} = e^{j \Delta \langle \hat{\bm{\pi}}_k, \boldsymbol{x}_i \rangle}$
\State $\bm{y}_{\text{vec}} \gets \text{flatten}(X_{\text{glob}})$
\State $\hat{\bm{A}} \gets A_{\text{glob}}^\dagger \bm{y}_{\text{vec}}$ \Comment{Solve $A_{\text{glob}} \hat{\bm{A}} \approx \bm{y}_{\text{vec}}$}

\State \Return $\hat{\bm{\pi}}, \hat{\bm{A}}$
\end{algorithmic}
\end{algorithm}